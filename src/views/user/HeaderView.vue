        ...
        <div class="nav-items">
          ...
          >
            ...
            <template v-if="menu.subMenus">
              ...
              <span class="v-icon" :class="{ 'rotate': nav_item_selected === menu.name }">
            ...
            <!-- 没有下拉菜单的情况 -->
            <template v-else>
              <router-link :to="menu.to" class="menu-title">{{ menu.title }}</router-link>
            </template>
...
const menus = ref([
  ...
  },
  {
    title: '全球天气',
    name: 'GlobalWeather',
    to: { name: 'GlobalWeather_ForecastResult' }
  },
  {
    title: 'NAO',
    name: 'NAO',
...
.logo_txt_w { opacity: 1; z-index: 1; }
.logo_txt_b { opacity: 0; z-index: 2; }
.header-container:hover .logo_txt_w { opacity: 0; }
.header-container:hover .logo_txt_b { opacity: 1; }
.menu-title {
...
.header-container:hover .menu-title { color: black; }
.v-icon {
  display: flex;
...
.v-icon svg path { fill: #e1e1e1; }
.header-container:hover .v-icon svg path { fill: black; }
.v-icon.rotate { transform: rotate(180deg); }
.header-container:hover .v-icon.rotate svg path { fill: #2d8dd2; }
.header-content {
  width: 100%;
  height: 70px;
  margin: 0 auto;
